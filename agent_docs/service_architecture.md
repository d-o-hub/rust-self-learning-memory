...
- `patterns/` - Pattern extraction and validation (~4,230 LOC)
  ...
  - Pattern ranking utilizes the Schwartzian Transform (decorate-sort-undecorate) to pre-calculate expensive keys (like `HashSet` allocations and `Utc::now()` calls). This reduces complexity from O(N log N) to O(N) scoring calls.
