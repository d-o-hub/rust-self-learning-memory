    ...
    fn from(pack: CompactHandoff) -> Self {
        ...
        Self {
            ...
            omitted: OmittedView {
                ...
                omitted_artifacts: pack.omitted.omitted_artifacts,
                truncated_fields: pack.omitted.truncated_fields,
                full_available_via: format!("episode handoff {} --full", pack.checkpoint_id),
            },
            payload_bytes,
        ...
        match format {
            ...
            OutputFormat::Human => {
                ...
                if !self.pattern_refs.is_empty() {
                    println!("Pattern References:");
                    for pat in &self.pattern_refs {
                        println!("  * {pat}");
                    }
                    println!();
                }

                if !self.heuristic_refs.is_empty() {
                    println!("Heuristic References:");
                    for h in &self.heuristic_refs {
                        println!("  * {h}");
                    }
                    println!();
                }
                ...
                if !self.evidence_excerpts.is_empty() {
                    ...
                    for excerpt in &self.evidence_excerpts {
                        if let Some(ref res) = excerpt.result_summary {
                            println!(
                                "  [step {}] {}: {} -> {}",
                                excerpt.step_number, excerpt.tool, excerpt.action, res
                            );
                        } else {
                            println!(
                                "  [step {}] {}: {}",
                ...
                let has_omitted_counts = omitted.omitted_steps > 0
                    ...
                    || omitted.omitted_artifacts > 0;
                let has_truncated_fields = !omitted.truncated_fields.is_empty();

                if has_omitted_counts || has_truncated_fields {
                    if has_omitted_counts {
                        println!(
                            "Omitted: {} steps, {} findings, {} decisions, {} actions, {} patterns, {} heuristics, {} artifacts",
                            ...
                            omitted.omitted_artifacts
                        );
                    }
                    if has_truncated_fields {
                        println!("Truncated fields: {}", omitted.truncated_fields.join(", "));
                    }
...
mod tests {
    ...
    #[test]
    fn compact_result_human_output_formatting() {
        use do_memory_core::memory::checkpoint::{
            CompactHandoff, EvidenceExcerpt, OmissionMetadata,
        };
        ...
        let pack = CompactHandoff {
            ...
            episode_id: uuid::Uuid::nil(),
            timestamp: chrono::Utc::now(),
            current_goal: "Truncated goal...".to_string(),
            status: "in_progress".to_string(),
            steps_done: 1,
            steps_total: 1,
            verified_findings: vec!["f1".to_string()],
            decisions: vec!["d1".to_string()],
            pending_actions: vec!["a1".to_string()],
            pattern_refs: vec!["pat-123".to_string()],
            heuristic_refs: vec!["heur-456".to_string()],
            artifact_refs: vec!["src/main.rs".to_string()],
            evidence_excerpts: vec![EvidenceExcerpt {
                step_number: 1,
            ...
            omitted: OmissionMetadata {
                omitted_steps: 0,
                truncated_fields: vec!["current_goal".to_string()],
                ..OmissionMetadata::default()
            },
            approx_tokens: 50,
        };
        ...
        let result = CompactHandoffResult::from(pack);
        // Ensure write(OutputFormat::Human) runs without error
        assert!(result.write(OutputFormat::Human).is_ok());
    }
}
