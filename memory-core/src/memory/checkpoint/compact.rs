...
use budget::{enforce_byte_budget, summarize_result, truncate_chars};
...
/// Default cap for artifact references.
pub const DEFAULT_MAX_ARTIFACT_REFS: usize = 5;
...
impl Default for HandoffBudget {
    fn default() -> Self {
        Self {
            ...
            max_artifact_refs: DEFAULT_MAX_ARTIFACT_REFS,
...
pub struct OmissionMetadata {
    ...
    /// Whether the full fidelity pack is available for fetch.
    #[serde(default = "default_true")]
    pub full_fidelity_available: bool,
}
...
pub fn assemble_compact(inputs: CompactInputs, budget: &HandoffBudget) -> CompactHandoff {
    ...
    // Fixed-point convergence loop: approx_tokens changes payload_bytes() when serialized.
    // Re-enforce byte budget and update approx_tokens until convergence (or iteration limit).
    for _ in 0..10 {
        enforce_byte_budget(&mut pack, budget);
        let bytes = pack.payload_bytes();
        let tokens = bytes / 4;
        if pack.approx_tokens == tokens {
            break;
        }
        pack.approx_tokens = tokens;
    }
    // Final enforcement pass against the serialized representation containing approx_tokens
    enforce_byte_budget(&mut pack, budget);
    pack.approx_tokens = pack.payload_bytes() / 4;
    pack
}
