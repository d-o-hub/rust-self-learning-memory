...
fn compact_payload_complies_with_tight_budget() {
    ...
    assert!(pack.omitted.full_fidelity_available);
...
#[test]
fn compact_budget_exact_boundary_convergence() {
    let inputs = sample_inputs(10);
    // Determine the pre-token payload size under a tight budget
    let budget_unbound = HandoffBudget {
        max_bytes: usize::MAX,
        ..HandoffBudget::default()
    };
    let raw_pack = assemble_compact(inputs.clone(), &budget_unbound);
    let full_size = raw_pack.payload_bytes();

    // Test exact boundary limits around full_size and tight limits like 1024
    for limit in [full_size - 50, full_size - 1, full_size, 1024, 1050, 1100] {
        let budget = HandoffBudget {
            max_bytes: limit,
            ..HandoffBudget::default()
        };
        let pack = assemble_compact(inputs.clone(), &budget);
        assert!(
            pack.payload_bytes() <= limit || pack.payload_bytes() < 1000,
            "payload_bytes {} exceeded limit {}",
            pack.payload_bytes(),
            limit
        );
        assert_eq!(
            pack.approx_tokens,
            pack.payload_bytes() / 4,
            "approx_tokens mismatch for limit {limit}: tokens={}, payload_bytes={}",
            pack.approx_tokens,
            pack.payload_bytes()
        );
    }
...
fn compact_sub_floor_budget_returns_best_effort_skeleton() {
    ...
    assert_eq!(pack.omitted.omitted_artifacts, 1);
}
