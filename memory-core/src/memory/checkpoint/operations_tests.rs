...
#[tokio::test]
async fn test_historical_checkpoint_snapshot_purity() {
    use crate::memory::MemoryConfig;
    ...
    let episode_id = start_episode_with_steps(&memory, "Multi-stage task", 3).await;
    let checkpoint = checkpoint_episode(&memory, episode_id, "mid-task pause".to_string())
        .await
        .unwrap();

    // Log 5 more steps after checkpoint and complete episode
    for i in 4..=8 {
        let step = crate::episode::ExecutionStep::new(
            i,
            "later_tool".to_string(),
            format!("later action {i}"),
        );
        memory.log_step(episode_id, step).await;
    }
    memory
        .complete_episode(
            episode_id,
            crate::types::TaskOutcome::Success {
                verdict: "Done later".to_string(),
                artifacts: vec!["final.rs".to_string()],
            },
        )
        .await
        .unwrap();

    // Retrieve pack for early checkpoint
    let pack =
        get_compact_handoff_pack(&memory, checkpoint.checkpoint_id, HandoffBudget::default())
            .await
            .unwrap();

    assert_eq!(pack.steps_done, 3);
    assert_eq!(pack.steps_total, 3);
    assert_eq!(pack.status, "in_progress");
    assert_eq!(pack.evidence_excerpts.len(), 3);
    assert!(
        pack.evidence_excerpts
            .iter()
            .all(|e| e.step_number <= 3 && !e.tool.contains("later"))
    );
}
