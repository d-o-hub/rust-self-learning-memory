...
fn test_success_analyzer_edge_cases() {
    ...
    assert!(successes
        .iter()
        .any(|s| s.contains("Partial success: Partial")));
    ...
    assert!(!successes
        .iter()
        .any(|s| s.contains("High execution success rate")));
}
...
fn test_success_patterns_edge_cases() {
    ...
    assert!(!patterns
        .iter()
        .any(|p| p.contains("Effective tool sequence")));
    ...
    assert!(!patterns
        .iter()
        .any(|p| p.contains("Successfully leveraged Rust-specific")));
}
...
fn test_improvement_analyzer_edge_cases() {
    ...
    assert!(!improvements
        .iter()
        .any(|i| i.contains("Optimize execution time")));
    ...
    assert!(improvements
        .iter()
        .any(|i| i.contains("Reduce failed execution steps")));
...
fn test_improvement_opportunities_edge_cases() {
    ...
    let mut s1 = successful_step(1, "tool", "action");
    s1.latency_ms = 100;
    let mut s2 = successful_step(2, "tool", "action");
    s2.latency_ms = 200; // avg = 150. max = 200. 200 < 450.
    let steps = vec![s1, s2];
    let outcome = TaskOutcome::Success {
        verdict: "ok".to_string(),
        artifacts: vec![],
    ...
    assert!(!opportunities
        .iter()
        .any(|o| o.contains("Performance bottleneck")));

    // Case 2: Redundancy just below threshold (count = 4)
    let steps: Vec<_> = (0..4)
        .map(|i| successful_step(i, "tool", "action"))
        .collect();
    ...
    assert!(!opportunities
        .iter()
        .any(|o| o.contains("Potential for parallelization")));

    // Case 4: Resource utilization normal (tokens between 1000 and 10000)
    let mut s1 = successful_step(1, "tool", "action");
    s1.tokens_used = Some(5000);
    let steps = vec![s1];
