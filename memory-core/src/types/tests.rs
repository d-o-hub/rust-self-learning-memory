...
#[serial]
#[test]
fn test_memory_config_candidate_budget_env_vars() {
    // Numeric budget
    unsafe {
        std::env::set_var("MEMORY_CANDIDATE_BUDGET", "50");
    }
    let config = MemoryConfig::from_env();
    assert_eq!(config.candidate_budget, Some(50));

    // "none" and "0" disable the budget
    for input in ["none", "NONE", "0"] {
        unsafe {
            std::env::set_var("MEMORY_CANDIDATE_BUDGET", input);
        }
        let config = MemoryConfig::from_env();
        assert_eq!(config.candidate_budget, None, "input {input}");
    }

    // Invalid input keeps the default
    unsafe {
        std::env::set_var("MEMORY_CANDIDATE_BUDGET", "not_a_number");
    }
    let config = MemoryConfig::from_env();
    assert_eq!(config.candidate_budget, Some(100));

    // Compatibility mode truthy spellings
    for input in ["true", "1", "yes", "on", "TRUE"] {
        unsafe {
            std::env::set_var("MEMORY_COMPATIBILITY_MODE", input);
        }
        let config = MemoryConfig::from_env();
        assert!(config.compatibility_mode, "input {input}");
    }

    // Falsy input leaves compatibility mode off
    unsafe {
        std::env::set_var("MEMORY_COMPATIBILITY_MODE", "false");
    }
    let config = MemoryConfig::from_env();
    ...
    // Cleanup
    unsafe {
        std::env::remove_var("MEMORY_CANDIDATE_BUDGET");
        std::env::remove_var("MEMORY_COMPATIBILITY_MODE");
    }
}
