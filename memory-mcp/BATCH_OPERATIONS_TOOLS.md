...
1. `memory-mcp/src/server/tool_definitions_extended.rs` (advertise tool schemas)
2. `memory-mcp/src/bin/server_impl/handlers.rs` (dispatch handlers)
3. `memory-mcp/tests/tool_contract_parity.rs` (parity and call-behavior checks)
4. Docs in `memory-mcp/README.md`, `docs/API_REFERENCE.md`, and plans status files
