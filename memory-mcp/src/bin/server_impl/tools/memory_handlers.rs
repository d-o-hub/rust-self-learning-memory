...
pub async fn handle_advanced_pattern_analysis(
    ...
    let mut time_series_data: std::collections::HashMap<String, Vec<f64>> =
        serde_json::from_value(time_series_data_value.clone())?;

    // Enforce series limits (CWE-770)
    if time_series_data.len() > do_memory_mcp::constants::MAX_SERIES_VARS {
        let mut keys: Vec<String> = time_series_data.keys().cloned().collect();
        keys.sort();
        for key in keys
            .into_iter()
            .skip(do_memory_mcp::constants::MAX_SERIES_VARS)
        {
            time_series_data.remove(&key);
        }
    }
    for series in time_series_data.values_mut() {
        series.truncate(do_memory_mcp::constants::MAX_SERIES_POINTS);
    }
...
pub async fn handle_get_metrics(
    ...
    let args: Value = arguments.unwrap_or(json!({}));
    let metric_type = args.get("metric_type").and_then(|v| v.as_str()).map(|s| {
        let mut s = s.to_string();
        let max = do_memory_mcp::constants::MAX_METRICS_TYPE_LEN;
        if s.len() > max {
            let mut end = max;
