...
mod performance_benchmarks {
    ...
    fn benchmark_compatibility_assessment() {
        ...
        for count in tool_counts {
            let tools: Vec<String> = registered_tools.iter().take(count).cloned().collect();
    ...
    fn benchmark_memory_usage() {
        ...
        let values: Vec<f64> = (0..size).map(|i| 10.0 + (i as f64 / size as f64) * 10.0).collect();
        let timestamps: Vec<f64> = (0..size).map(|i| i as f64).collect();

        use rand::seq::SliceRandom;
        let mut indexed_values: Vec<(f64, f64)> = values.into_iter().zip(timestamps.into_iter()).collect();
        ...
        let estimated_mb = values_capacity_mb + timestamps_capacity_mb + 5.0;
        ...
        assert!(
            duration.as_secs() < 30,
            "DBSCAN with {} points should complete in < 30s",
            size
        );
    ...
    fn benchmark_streaming_performance() {
        ...
        for window_size in window_sizes {
            ...
            assert!(
                throughput > 50.0,
                "Streaming should process at least 50 points/sec"
            );
        }
    }
