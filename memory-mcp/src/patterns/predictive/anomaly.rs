...
//!
//! Anomaly detection using DBSCAN clustering and statistical methods.
...
use super::types::{AnomalyResult, PredictiveConfig};
...
impl AnomalyDetector {
    ...
    pub fn with_config(config: PredictiveConfig) -> Result<Self> {
        let dbscan_config = DBSCANConfig {
            density: 0.1 * config.anomaly_sensitivity,
            min_cluster_size: 3,
            max_distance: 1.0,
    ...
    pub fn detect_anomalies(&mut self, data: &HashMap<String, Vec<f64>>) -> Result<Vec<AnomalyResult>> {
    ...
    fn detect_variable_anomalies(&mut self, variable: &str, series: &[f64]) -> Result<AnomalyResult> {
        ...
        for (i, &label) in cluster_labels.iter().enumerate() {
            match label {
                ClusterLabel::Noise => {
                    ...
                    let deviation = (series[i] - series.iter().sum::<f64>() / series.len() as f64).abs();
                    let variance: f64 = series.iter().map(|&x| {
                        let mean = series.iter().sum::<f64>() / series.len() as f64;
                        (x - mean).powi(2)
                    }).sum::<f64>() / series.len() as f64;
                    let std_dev = variance.sqrt();
                    anomaly_scores.push(if std_dev > 0.0 { deviation / std_dev } else { 1.0 });
                }
        ...
        let confidence = if !series.is_empty() {
            let cluster_count = cluster_labels.iter().filter(|&label| !matches!(label, ClusterLabel::Noise)).count();
...
mod tests {
    ...
    fn test_dbscan_integration_with_anomaly_detector() -> Result<()> {
        let mut detector = AnomalyDetector::new()?;
        let mut data = HashMap::new();
        data.insert("test_series".to_string(), vec![1.0, 1.1, 0.9, 1.0, 0.95, 1.05, 25.0, 1.0, 0.98, 1.02, 1.1, 0.89]);
        let anomalies = detector.detect_anomalies(&data)?;
        assert!(!anomalies.is_empty());
