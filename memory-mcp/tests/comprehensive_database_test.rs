...
async fn test_comprehensive_database_operations() {
    ...
    // Log execution steps
    let steps = vec![
        ExecutionStep::new(1, "create_html".to_string(), "Create PWA HTML structure".to_string()),
        ExecutionStep::new(2, "add_manifest".to_string(), "Add Web App Manifest".to_string()),
        ExecutionStep::new(3, "implement_service_worker".to_string(), "Implement service worker for offline support".to_string()),
        ExecutionStep::new(4, "add_local_storage".to_string(), "Implement localStorage for data persistence".to_string()),
    ];
    ...
    let outcome = TaskOutcome::Success {
        verdict: "PWA Todo List implemented successfully with local storage and offline support".to_string(),
        artifacts: vec![
    ...
    let query_result = mcp_server.query_memory(
        "PWA todo list".to_string(),
        "pwa".to_string(),
        None,
        10
    ).await.unwrap();
    ...
    assert_eq!(episode["task_description"], "Build PWA Todo List with Local Storage");
    ...
    println!("✅ Memory query returned {} episodes with {} patterns", episodes.len(), query_result["patterns"].as_array().unwrap().len());
    ...
    println!("   📈 Statistics: {} total, avg success rate: {:.2}",
    ...
    let _ = mcp_server.query_memory("test".to_string(), "test".to_string(), None, 1).await.unwrap();
    let _ = mcp_server.analyze_patterns("test".to_string(), 0.5, 5).await.unwrap();
    ...
    println!("✅ Tool usage tracking verified: {} tools tracked", usage.len());
    ...
    let code = r#"
        ...
        return {
            ...
            message: "PWA Todo functionality working!"
        };
    "#;

    let context = ExecutionContext::new(
        "Test PWA todo logic".to_string(),
        json!({"test": "pwa-functionality"})
    );
    ...
    println!("✅ Execution statistics: {} total, {} successful, {} failed",
    ...
    let verify_result = mcp_server.query_memory(
        "PWA todo list".to_string(),
        "pwa".to_string(),
        None,
        10
    ...
    assert_eq!(verify_episode["task_description"], "Build PWA Todo List with Local Storage");
    ...
    println!("   - Security violations: {}", final_stats.security_violations);

    // Test 8: Memory Pattern Retrieval
    println!("\n🧠 Test 8: Memory Pattern Retrieval");

    let patterns_result = memory.retrieve_relevant_patterns(
        &TaskContext {
            domain: "pwa".to_string(),
            language: Some("javascript".to_string()),
            framework: Some("vanilla-js".to_string()),
    ...
    println!("✅ Pattern retrieval completed - found {} patterns", patterns_result.len());
