...
use memory_core::{SelfLearningMemory, TaskContext, ExecutionStep, TaskOutcome, TaskType, ComplexityLevel};
...
mod pwa_integration_tests {
    ...
    async fn setup_pwa_test_environment() -> (Arc<SelfLearningMemory>, Arc<MemoryMCPServer>) {
    ...
    #[tokio::test]
    async fn test_pwa_todo_app_workflow_database_entries() {
        println!("🧪 Testing PWA Todo App Workflow - Database Entries");
        println!("==================================================");

        let (memory, mcp_server) = setup_pwa_test_environment().await;

        // Simulate PWA Todo App workflow
        println!("\n1. Episode Creation - PWA Development");
        println!("-------------------------------------");

        let episode_id = memory.start_episode(
            "Build PWA Todo List with Local Storage".to_string(),
            TaskContext {
                domain: "pwa".to_string(),
                language: Some("javascript".to_string()),
                framework: Some("vanilla-js".to_string()),
                complexity: ComplexityLevel::Moderate,
                tags: vec!["pwa".to_string(), "todo".to_string(), "local-storage".to_string()],
            },
        ...
        // Log development steps (simulating PWA development)
        println!("\n2. Development Steps Logging");
        println!("----------------------------");

        let steps = vec![
            ("create_html", "Create PWA HTML structure with modern JavaScript"),
            ("add_manifest", "Add Web App Manifest for PWA installation"),
            ("implement_service_worker", "Implement service worker for offline functionality"),
            ("add_local_storage", "Implement localStorage for data persistence"),
            ("add_responsive_design", "Add responsive CSS and mobile-first design"),
            ("test_pwa_features", "Test PWA installation and offline capabilities"),
        ];
        ...
        let outcome = TaskOutcome::Success {
            verdict: "PWA Todo List implemented successfully with local storage, service worker, and offline support. All PWA features working correctly.".to_string(),
            artifacts: vec![
        ...
        let episodes_result = mcp_server.query_memory(
            "PWA todo".to_string(),
            "pwa".to_string(),
            None,
            10
        ...
        println!("   Steps Count: {}", episode["steps"].as_array().unwrap().len());
        println!("   Status: Complete");
        println!("   Artifacts: {:?}", episode["outcome"]["Success"]["artifacts"]);

        // Test 2: Verify Pattern Extraction
        println!("\n5. Database Verification - Patterns");
        println!("-----------------------------------");

        let patterns_result = mcp_server.analyze_patterns(
        ...
        for _ in 0..3 {
            let _ = mcp_server.query_memory("test".to_string(), "test".to_string(), None, 1).await;
        }
        let _ = mcp_server.analyze_patterns("test".to_string(), 0.5, 2).await;
        let _ = mcp_server.execute_agent_code("return 42;".to_string(), ExecutionContext::new("test".to_string(), json!({}))).await;
        ...
        let memory_patterns = memory.retrieve_relevant_patterns(
            &TaskContext {
                domain: "pwa".to_string(),
                language: Some("javascript".to_string()),
                framework: Some("vanilla-js".to_string()),
        ...
        for (i, pattern) in memory_patterns.iter().enumerate() {
            println!("   Pattern {}: ID={}, Confidence={:.2}",
                    i + 1,
                    pattern.id(),
                    pattern.confidence());
        }
        ...
        let final_query = mcp_server.query_memory(
            "pwa".to_string(),
            "pwa".to_string(),
            None,
            5
        ...
        println!("   Total Episodes in Insights: {}", final_insights["total_episodes"]);
        println!("   Success Rate: {:.2}", final_insights["success_rate"]);

        println!("\n✅ PWA Todo App Database Verification Complete!");
        println!("=================================================");
        println!("All database entries verified and logged successfully.");
        println!("The PWA Todo App integration with Memory MCP is fully functional.");
    }

    #[tokio::test]
    async fn test_pwa_user_interaction_simulation() {
        println!("🎮 Testing PWA User Interaction Simulation");
        println!("==========================================");

        let (_memory, mcp_server) = setup_pwa_test_environment().await;

        // Simulate user interactions with the PWA
        println!("\n1. User Adds Todos");
        println!("------------------");

        // Simulate JavaScript code execution from PWA
        let add_todo_code = r#"
// Simulate adding todos in PWA
const todos = [
    { id: 1, text: "Learn PWA development", completed: false },
...
"#;
        ...
        let context = ExecutionContext::new(
            "PWA user adds multiple todos".to_string(),
            json!({"user_action": "bulk_add", "timestamp": "2025-11-15T10:00:00Z"})
        );

        match mcp_server.execute_agent_code(add_todo_code.to_string(), context).await {
            Ok(result) => {
                match result {
...
"#;
        ...
        let context = ExecutionContext::new(
            "PWA user completes a todo".to_string(),
            json!({"todo_id": 2, "action": "toggle_complete"})
        );

        match mcp_server.execute_agent_code(complete_todo_code.to_string(), context).await {
            Ok(result) => {
                match result {
                    memory_mcp::ExecutionResult::Success { output, .. } => {
...
const allTodos = [
    { id: 1, text: "Learn PWA", completed: false },
...
"#;
        ...
        let context = ExecutionContext::new(
            "PWA user filters todos by active status".to_string(),
            json!({"filter": "active", "total_todos": 3})
        );

        match mcp_server.execute_agent_code(filter_todos_code.to_string(), context).await {
            Ok(result) => {
                match result {
                    memory_mcp::ExecutionResult::Success { output, .. } => {
...
"#;
        ...
        let context = ExecutionContext::new(
            "PWA user clears all completed todos".to_string(),
            json!({"action": "clear_completed", "confirmed": true})
        );

        match mcp_server.execute_agent_code(clear_completed_code.to_string(), context).await {
            Ok(result) => {
                match result {
                    memory_mcp::ExecutionResult::Success { output, .. } => {
        ...
        println!("\n✅ PWA User Interaction Simulation Complete!");
        println!("=============================================");
