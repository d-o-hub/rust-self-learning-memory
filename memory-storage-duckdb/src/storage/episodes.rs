...
impl DuckDbStorage {
    ...
    pub(crate) async fn cleanup_episodes_internal(
        &self,
        _policy: &do_memory_core::episodic::EpisodeRetentionPolicy,
    ) -> Result<do_memory_core::episodic::CleanupResult> {
        // Basic implementation: delete episodes with very low reward
        let conn_arc = Arc::clone(&self.conn);
        ...
        let mut result = do_memory_core::episodic::CleanupResult::new();
        result.deleted_count = deleted_count;
        Ok(result)
    }

    pub(crate) async fn count_cleanup_candidates_internal(
        &self,
        _policy: &do_memory_core::episodic::EpisodeRetentionPolicy,
    ) -> Result<usize> {
        let conn_arc = Arc::clone(&self.conn);
