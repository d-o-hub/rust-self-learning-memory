//! Adaptive LRU cache entry with TTL tracking (metadata only)
//!
//! This module provides cache entries that track access patterns and adapt TTL
//! without storing actual values. This allows integration with RedbStorage
...
/// Internal cache entry with adaptive TTL tracking (metadata only, no value)
#[derive(Debug)]
pub(crate) struct AdaptiveLRUEntry {
...
impl AdaptiveLRUEntry {
    /// Create a new cache entry
    pub(crate) fn new(default_ttl: Duration, size_bytes: usize) -> Self {
        let now = Instant::now();
        Self {
            created_at: now,
            last_accessed: now,
...
impl Clone for AdaptiveLRUEntry {
    fn clone(&self) -> Self {
        Self {
