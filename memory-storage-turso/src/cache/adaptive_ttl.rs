...
struct HybridEvictionState {
    ...
    lru_scores: Vec<(Uuid, Uuid)>, // (id, last_access_nanos)
...
impl AdaptiveTtlStats {
    ...
    fn update_hit_rate(&self) {
        ...
        self.hit_rate.store(rate as u64, Ordering::Relaxed);
    }

    /// Get current hit rate as percentage
    pub fn hit_rate_percent(&self) -> f64 {
        self.hit_rate.load(Ordering::Relaxed) as f64
    }
...
impl<V: Clone + Send + Sync + 'static> AdaptiveTtlCache<V> {
    ...
    pub async fn effectiveness_report(&self) -> CacheEffectivenessReport {
        ...
        let pressure_evictions = self.stats.pressure_evictions.load(Ordering::Relaxed);
        let ttl_adaptations = self.stats.ttl_adaptations.load(Ordering::Relaxed);
    ...
    pub async fn get_stats(&self) -> AdaptiveTtlStatsSnapshot {
        ...
        AdaptiveTtlStatsSnapshot {
            ...
            pressure_evictions: self.stats.pressure_evictions.load(Ordering::Relaxed),
            ...
            ttl_adaptations: self.stats.ttl_adaptations.load(Ordering::Relaxed),
    ...
    fn start_pressure_task(
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                let heap_size = Self::estimate_heap_size();
                ...
                if state.pressure_level != new_level {
                    ...
                    stats
                        .current_pressure
                        .store(atomic_level, Ordering::Relaxed);
