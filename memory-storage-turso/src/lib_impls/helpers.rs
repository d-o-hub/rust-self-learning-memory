...
impl TursoStorage {
    ...
    pub fn enable_episode_cache(
        &mut self,
        config: TTLConfig,
    ) -> Result<Option<AdaptiveTTLCache<String, memory_core::Episode>>> {
        let cache = AdaptiveTTLCache::new(config)
            .map_err(|e| Error::Storage(format!("Failed to create adaptive TTL cache: {}", e)))?;
    ...
    /// ```
    #[cfg(feature = "adaptive-ttl")]
    pub fn enable_episode_cache_default(&mut self) -> Result<()> {
        let cache = AdaptiveTTLCache::default_config()
            .map_err(|e| Error::Storage(format!("Failed to create adaptive TTL cache: {}", e)))?;
    ...
    /// The previous cache if one existed, or None
    #[cfg(feature = "adaptive-ttl")]
    pub fn disable_episode_cache(
        &mut self,
    ) -> Option<AdaptiveTTLCache<String, memory_core::Episode>> {
