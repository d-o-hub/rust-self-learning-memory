...
//!
//! This module contains the core implementation of adaptive connection pool.
...
pub(super) struct AdaptiveMetrics {
    ...
    pub(super) last_scale_up: AtomicU64,
    pub(super) last_scale_down: AtomicU64,
}
...
pub struct AdaptiveConnectionPool {
    ...
    pub(super) config: Arc<AdaptivePoolConfig>,
    pub(super) semaphore: Arc<Semaphore>,
    pub(super) current_max: Arc<AtomicU32>,
...
impl AdaptiveConnectionPool {
    pub async fn new(
        ...
        let conn = pool
            .db
            .connect()
            .map_err(|e| memory_core::Error::Storage(format!("Failed to connect: {}", e)))?;
        conn.query("SELECT 1", ()).await.map_err(|e| {
            memory_core::Error::Storage(format!("Database validation failed: {}", e))
        })?;
    ...
    async fn try_acquire(&self, timeout: Duration) -> memory_core::Result<OwnedSemaphorePermit> {
        ...
        match tokio::time::timeout(timeout, self.semaphore.clone().acquire_owned()).await {
            ...
            Ok(Err(e)) => Err(memory_core::Error::Storage(format!(
                "Failed to acquire connection permit: {}",
                e
            ))),
            Err(_) => Err(memory_core::Error::Storage(format!(
    ...
    pub async fn get(&self) -> memory_core::Result<AdaptivePooledConnection> {
