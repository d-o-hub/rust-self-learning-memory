...
impl TursoStorage {
    ...
    pub async fn enforce_capacity(&self, max_episodes: usize) -> Result<()> {
        ...
        self._delete_embeddings_batch_internal(&evicted).await?;
        ...
        // Build params using more idiomatic mapping
        let params: Vec<libsql::Value> = evicted.iter().cloned().map(Into::into).collect();
