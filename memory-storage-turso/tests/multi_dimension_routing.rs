...
#[tokio::test]
#[ignore = "Waiting for multi-dimension routing implementation"]
async fn test_384_dimension_routing() {
    // This test will verify that 384-dim embeddings go to embeddings_384 table
    let harness = MultiDimensionTestHarness::new().await.expect("Failed to create harness");
    
    let (episode, embedding) = harness.create_episode_with_embedding(384, 42)
        .await
        .expect("Failed to create episode with embedding");
    
    // Verify the embedding was stored in the correct table
    let in_correct_table = harness.verify_table_usage(episode.episode_id, 384)
        .await
        .expect("Failed to verify table usage");
    
    assert!(in_correct_table, "384-dim embedding should be in embeddings_384 table");
}
...
async fn test_1536_dimension_routing() {
    ...
    let (episode, embedding) = harness.create_episode_with_embedding(1536, 42)
        .await
        .expect("Failed to create episode with embedding");
    
    let in_correct_table = harness.verify_table_usage(episode.episode_id, 1536)
        .await
        .expect("Failed to verify table usage");
    
    assert!(in_correct_table, "1536-dim embedding should be in embeddings_1536 table");
}
...
async fn test_unsupported_dimension_routing() {
    ...
    let (episode, embedding) = harness.create_episode_with_embedding(512, 42)
        .await
        .expect("Failed to create episode with embedding");
    
    let in_correct_table = harness.verify_table_usage(episode.episode_id, 512)
        .await
        .expect("Failed to verify table usage");
    
    assert!(in_correct_table, "512-dim embedding should be in embeddings_other table");
}
...
async fn test_mixed_dimension_routing() {
    ...
    for (i, &dim) in dimensions.iter().enumerate() {
        let (episode, _) = harness.create_episode_with_embedding(dim, 42 + i as u64)
            .await
            .expect("Failed to create episode with embedding");
        
        let in_correct_table = harness.verify_table_usage(episode.episode_id, dim)
            .await
            .expect("Failed to verify table usage");
...
async fn test_backward_compatibility_embedding_retrieval() {
    ...
    let (storage, _dir) = create_test_storage().await.expect("Failed to create storage");
    ...
    let retrieved = storage.get_episode_embedding(episode.episode_id)
        .await
...
async fn test_vector_search_across_dimensions() {
    ...
    for (i, &dim) in dimensions.iter().enumerate() {
        let (episode, embedding) = harness.create_episode_with_embedding(dim, 100 + i as u64)
            .await
            .expect("Failed to create episode with embedding");
        
        // Search for similar episodes (should find the one we just created)
        let results = harness.run_similarity_search(embedding.clone(), 5, 0.5)
            .await
            .expect("Failed to run similarity search");
        
        assert!(!results.is_empty(), "Should find similar episodes for {} dimension", dim);
        
        // The most similar should be the episode itself (if threshold allows)
        let found = results.iter().any(|(id, _)| *id == episode.episode_id);
        assert!(found, "Should find the episode we just created for {} dimension", dim);
    }
}