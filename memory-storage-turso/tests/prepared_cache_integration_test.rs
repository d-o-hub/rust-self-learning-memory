...
async fn test_prepared_statement_cache_integration() {
    ...
    assert_eq!(stats_after_store.current_size, 0, "Cache cleared after operation");
    ...
    assert_eq!(
        retrieved2.unwrap().task_description,
        "Second test episode"
    );

    let stats_final = storage.prepared_cache_stats();
    assert_eq!(stats_final.current_size, 0, "Cache cleared after operations");
