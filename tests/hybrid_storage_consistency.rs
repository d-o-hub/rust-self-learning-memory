...
use do_memory_test_utils::{hybrid_memory, assert_episode_parity};
...
async fn test_hybrid_storage_write_parity() {
    ...
    memory.log_step(episode_id, ExecutionStep::new(1, "tool".to_string(), "action".to_string())).await;
    ...
    memory.complete_episode(episode_id, outcome).await.expect("Complete episode");

    // 2. Fetch from the memory orchestrator
    let orchestrator_episode = memory.get_episode(episode_id).await.expect("Get from orchestrator");
    ...
    let turso_episode = turso.get_episode(episode_id).await.expect("Get from Turso").expect("Episode not in Turso");
    let redb_episode = redb.get_episode(episode_id).await.expect("Get from redb").expect("Episode not in redb");
...
async fn test_hybrid_storage_update_parity() {
    ...
    // complete it first
    memory.complete_episode(episode_id, TaskOutcome::Success { verdict: "ok".to_string(), artifacts: vec![] }).await.unwrap();

    // update it
    let mut new_metadata = std::collections::HashMap::new();
    new_metadata.insert("test_key".to_string(), "test_value".to_string());

    memory.update_episode(episode_id, Some("Updated description".to_string()), Some(new_metadata)).await.expect("Update episode");

    // Verify both backends updated
    let (turso, redb) = memory.storage_backends();
    let turso_ep = turso.unwrap().get_episode(episode_id).await.unwrap().unwrap();
    let redb_ep = redb.unwrap().get_episode(episode_id).await.unwrap().unwrap();
...
async fn test_hybrid_storage_delete_parity() {
    ...
    // delete it
    memory.delete_episode(episode_id).await.expect("Delete episode");

    // Verify both backends deleted
    let (turso, redb) = memory.storage_backends();
    assert!(turso.unwrap().get_episode(episode_id).await.unwrap().is_none());
    assert!(redb.unwrap().get_episode(episode_id).await.unwrap().is_none());
}
...
async fn test_hybrid_storage_query_parity() {
    ...
    let (turso, redb) = memory.storage_backends();
    let turso_episodes = turso.unwrap().query_episodes_since(chrono::Utc::now() - chrono::Duration::hours(1), None).await.unwrap();
    let redb_episodes = redb.unwrap().query_episodes_since(chrono::Utc::now() - chrono::Duration::hours(1), None).await.unwrap();
