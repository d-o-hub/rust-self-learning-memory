...
use do_memory_core::{
    Episode, Heuristic, Pattern, Result, StorageBackend, TaskContext, TaskType,
    TaskOutcome, MemoryConfig, Error,
};
use do_memory_core::episode::{
    Direction, EpisodeRelationship, EpisodePatternRelationship,
    RelationshipType, EpisodeRetentionPolicy, CleanupResult, PatternId,
};
...
use do_memory_test_utils::{temp_local_storage, in_memory_redb_storage};
...
impl StorageBackend for FailingStorage {
    ...
    async fn query_episodes_since(&self, _since: DateTime<Utc>, _limit: Option<usize>) -> Result<Vec<Episode>> {
        Err(Error::Storage("Simulated failure".to_string()))
    }
    async fn query_episodes_by_metadata(&self, _key: &str, _value: &str, _limit: Option<usize>) -> Result<Vec<Episode>> {
        Err(Error::Storage("Simulated failure".to_string()))
    }
    ...
    async fn get_recommendation_session(&self, _session_id: Uuid) -> Result<Option<RecommendationSession>> {
        Err(Error::Storage("Simulated failure".to_string()))
    }
    async fn get_recommendation_session_for_episode(&self, _episode_id: Uuid) -> Result<Option<RecommendationSession>> {
        Err(Error::Storage("Simulated failure".to_string()))
    }
    async fn store_recommendation_feedback(&self, _feedback: &RecommendationFeedback) -> Result<()> {
        Err(Error::Storage("Simulated failure".to_string()))
    }
    async fn get_recommendation_feedback(&self, _session_id: Uuid) -> Result<Option<RecommendationFeedback>> {
        Err(Error::Storage("Simulated failure".to_string()))
    }
...
async fn test_recovery_turso_unvailable_on_read() {
    ...
    let episode = Episode::new("Cached episode".to_string(), TaskContext::default(), TaskType::Testing);
    ...
    let retrieved = memory.get_episode(episode_id).await.expect("Should return episode from cache");
...
async fn test_reconciliation_stale_cache() {
    ...
    let episode = Episode::new("Turso only episode".to_string(), TaskContext::default(), TaskType::Testing);
    let episode_id = episode.episode_id;

    let (turso_backend, redb_backend) = memory.storage_backends();
    turso_backend.unwrap().store_episode(&episode).await.unwrap();

    // Verify it's NOT in redb yet
    assert!(redb_backend.as_ref().unwrap().get_episode(episode_id).await.unwrap().is_none());

    // 2. Read through the memory orchestrator
    // This should trigger the reconciliation logic (fetch from Turso, then store in redb)
    let retrieved = memory.get_episode(episode_id).await.expect("Should retrieve from Turso");
    assert_eq!(retrieved.episode_id, episode_id);
...
async fn test_recovery_turso_failure_on_write_does_not_block() {
    ...
    assert!(result.is_ok(), "complete_episode should be resilient to single backend failure");
